...
pub struct Validator<Claims: DeserializeOwned> {
    authority: Authority<Claims>,
    cached_key_set: Mutex<CachedKeySet>,
}

impl<Claims: DeserializeOwned> Validator<Claims> {
    pub async fn new(authority: Authority<Claims>) -> Result<Self, reqwest::Error> {
        ...
        if let Ok(header) = decode_header(jwt) {
            if let Some(thumbprint) = header.kid {
                if let Some(key) = self.get_key(&thumbprint).await {
                    ...
                    return decode::<Claims>(jwt, &decoding_key, &validation).is_ok();
                }
            }
        }
